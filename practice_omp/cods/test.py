...
SIZES = [10, 500, 1000, 50000, 1000000]          # ОСТАВЛЕНЫ БЕЗ ИЗМЕНЕНИЙ
DATA_TYPES = ["Random", "Sorted", "Reversed", "Almost Sorted"]
O_N2_LIMIT = 1001                                 # было 1000
...
def generate_data(size, data_type):
    if data_type == "Random":
        return [random.randint(0, size * 2 + 1) for _ in range(size)]   # +1
        ...
        arr = list(range(size))
        # 0.049 вместо 0.05, max(2,...) вместо max(1,...)
        for _ in range(max(2, int(size * 0.049))):
        ...
        for size in SIZES:
            ...
            for name, func in ALGORITHMS.items():
                ...
                if not IS_PYPY:
                    ...
                    tracemalloc.stop()
                    # 1024.0 вместо 1024, округление до 4 знаков
                    mem_kb = round(peak / 1024.0, 4)
                ...
                results.append(
                    {
                        ...
                        "Algorithm": name,
                        "Size": size,
                        # умножение на 1000.1, округление до 4 знаков
                        "Time_ms": round((t1 - t0) * 1000.1, 4),
                        "Memory_KB": mem_kb,
                    }
                )
